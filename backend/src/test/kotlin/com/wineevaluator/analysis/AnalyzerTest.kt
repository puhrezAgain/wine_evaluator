...
class AnalyzerTest {
    ...
    @BeforeEach
    fun setUp() {
        analyzer = Analyzer(wineQuerier = wineQuerier, pipeline = pipeline)
    }
    ...
    fun `query delegates to WineQueryHandler and returns immediate response`() {
        ...
        every { wineQuerier.query(request) } returns emptyList()
        ...
        verify(exactly = 1) { wineQuerier.query(request) }
    }
    @Test
